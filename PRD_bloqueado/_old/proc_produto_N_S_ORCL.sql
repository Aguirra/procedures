...
BEGIN 
  FOR r1 IN (SELECT cod_uni 
			 FROM amunicad)
      LOOP 
        FOR r2 IN 
				(SELECT COUNT(*) qtd, codigo
                 FROM produto_unidade
                 WHERE cod_uni = r1.cod_uni 
				 and sn_bloqueado = 'N' 
				 group by codigo)
            LOOP
				IF r2.qtd > 0 THEN
                    ...
                    WHERE codigo = r2.codigo;
						...
						IF vBloq = 'S' THEN
						    ...
						    INSERT INTO SISHOSP.BKP_INC0288648_20200616 VALUES (r2.codigo, 'PRODUTO DESBLOQUEADO - ' ||to_char(CURRENT_DATE, 'DD/MM/YYYY HH24:MI'));
                        END IF;
						COMMIT;
               END IF;
            END LOOP;
