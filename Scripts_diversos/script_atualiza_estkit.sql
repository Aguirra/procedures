...
2. Executar script script_acerto_estkit -> esse script é para verificar se existem itens com quantidades de reservas de itens de kit maior que a quantidade disponível no estoque. Caso tenham é necessário fazer acerto de entrada para estes itens.
...
O procedimento acima descrito deve ser feito após a atualização do passo que será expedido.
*/
...
-- produtos que estão em kit nao usado
...
-- produtos que estão em kit nao usado e não CLV
...
-- produtos que estão em kit nao usado e não CLV e a reserva do kit não condiz com a quantidade montada
FOREACH
...
-- produtos que estão em kit nao usado e CLV
SELECT cod_set, cod_prd, COD_BARRA_INT, sum(quant) quant
  FROM tab_prod
...
-- produtos que estão em kit nao usado e CLV e a reserva do kit não condiz com a quantidade montada
FOREACH
...
-- verifica se existe algum produto que não CLV e tem estoque reservado para o kit e não está montado
UPDATE esestcad 
   SET est_kit = 0, 
       controle = DECODE (controle, '0', '1', '0')
...
-- verifica se existe algum produto com estoque reservado para o kit e não está montado
UPDATE esestcad 
   SET est_kit = 0, 
       controle = DECODE (controle, '0', '1', '0')
 WHERE nvl(est_kit,0) > 0 
...
-- verifica se existe algum produto com estoque reservado para o kit e não atualiza estoque
UPDATE esestcad
   SET est_kit = 0,
       controle = DECODE (controle, '0', '1', '0')
 WHERE est_kit > 0
...
--1) Estrutura das tabelas dos produtos que controlam e não controlam lote validade
