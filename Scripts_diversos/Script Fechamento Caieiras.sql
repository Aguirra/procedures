...
drop table dif;
create table dif(cod_prd CHAR(6) NOT NULL,
                      cod_set CHAR(4),
                      cod_barra_int CHAR(9),
...
SELECT ESHISCAD.COD_PRD,
       ...
       CASE 
         ...
         WHEN ESPROD_SET_LOTE.DATA_HORA_INC < TO_DATE('01/02/2014 00:00:00', '%d/%m/%Y %H:%M:%S') THEN COD_BARRA_INT
         ELSE ' '
       END COD_BARRA_INT,
       SUM(EST_ATU_MES) EST_ATU 
  FROM ESHISCAD, outer(ESPROD_SET_LOTE)
 WHERE ESHISCAD.ANO_MES='201401' 
